...
if (isset($_POST['seed']) || (php_sapi_name() === 'cli' && isset($argv[1]) && $argv[1] === 'seed')) {
    try {
        ...
        $insertVhv->execute(['1001', 'นางใจดี รักสงบ (ประธาน)', 1, '34180101', '10688', $passHash, 1]);
        $insertVhv->execute(['1002', 'นางสมศรี มีสุข (อสม.)', 1, '34180101', '10688', $passHash, 0]);
        $insertVhv->execute(['1003', 'นายสมชาย แข็งแรง (อสม.)', 2, '34180102', '10688', $passHash, 0]);
        ...
        $insertStgDm->execute(['10688', 'รพ.สต.ตาลสุม', '11', '1234567890111', 'นายดำ', 'เก่งจริง', '1', '1965-04-12', '111', '12 ม.1', '34180101', '1', '5', 'ผู้ป่วยเบาหวานเดิม']);
        // DM screening risk group
        $insertStgDm->execute(['10688', 'รพ.สต.ตาลสุม', '12', '1234567890112', 'นางแดง', 'งามยิ่ง', '2', '1972-08-23', '112', '45 ม.1', '34180101', '1', '2', 'กลุ่มเสี่ยงเบาหวาน']);
        // Normal DM target
        $insertStgDm->execute(['10688', 'รพ.สต.ตาลสุม', '13', '1234567890113', 'น.ส.เขียว', 'สดใส', '2', '1985-11-05', '113', '78/1 ม.1', '34180101', '1', '1', 'ปกติ']);
        ...
        // HT high risk target
        $insertStgHt->execute(['10688', 'รพ.สต.ตาลสุม', '12', '1234567890112', 'นางแดง', 'งามยิ่ง', '2', '1972-08-23', '112', '45 ม.1', '34180101', '1', 145, 95, '2']);
        $insertStgHt->execute(['10688', 'รพ.สต.ตาลสุม', '13', '1234567890113', 'น.ส.เขียว', 'สดใส', '2', '1985-11-05', '113', '78/1 ม.1', '34180101', '1', 120, 80, '1']);
        // Target in Moo 2
        $insertStgHt->execute(['10688', 'รพ.สต.ตาลสุม', '14', '1234567890114', 'นายขาว', 'บริสุทธิ์', '1', '1958-02-17', '201', '3 ม.2', '34180102', '1', 135, 85, '2']);
        ...
        $insertTarget->execute([
            '1234567890112', '112', '12', 'นางแดง', 'งามยิ่ง', '2', '1972-08-23', '45', 1, '341801', '34180101', '10688',
            15.4300, 104.9800, 'BOTH', 1, 1
        ]);
        
        // น.ส.เขียว สดใส (Moo 1) - home location: 15.4320, 104.9820
        $insertTarget->execute([
            '1234567890113', '113', '13', 'น.ส.เขียว', 'สดใส', '2', '1985-11-05', '78/1', 1, '341801', '34180101', '10688',
            15.4320, 104.9820, 'BOTH', 1, 1
        ]);
        
        // นายขาว บริสุทธิ์ (Moo 2) - home location: 15.4400, 104.9900
        $insertTarget->execute([
            '1234567890114', '201', '14', 'นายขาว', 'บริสุทธิ์', '1', '1958-02-17', '3', 2, '341801', '34180102', '10688',
            15.4400, 104.9900, 'HT_ONLY', 1, 1
        ]);
...
<body class="admin-body">
    ...
    <div style="max-width: 700px; margin: 60px auto; padding: 0 20px;">
        <div class="card-dark" style="text-align: center;">
            <h2 style="color: var(--color-accent); margin-bottom: 20px;">📂 ใส่ข้อมูลจำลองสำหรับผู้ทดสอบระบบ (Seed DB)</h2>
            ...
            <ul style="color: var(--text-primary); text-align: left; line-height: 2; margin-bottom: 30px; box-shadow: var(--neumorph-inset); background-color: var(--bg-card); padding: 20px 40px; border-radius: var(--border-radius); border: none; list-style-type: square;">
                <li><strong>ประธาน อสม. (Moo 1):</strong> รหัส อสม.: <code style="color: var(--color-accent);">1001</code> รหัสผ่าน: <code style="color: var(--color-accent);">1234</code></li>
                <li><strong>อสม. ทั่วไป (Moo 1):</strong> รหัส อสม.: <code style="color: var(--color-accent);">1002</code> รหัสผ่าน: <code style="color: var(--color-accent);">1234</code></li>
                <li><strong>อสม. ทั่วไป (Moo 2):</strong> รหัส อสม.: <code style="color: var(--color-accent);">1003</code> รหัสผ่าน: <code style="color: var(--color-accent);">1234</code></li>
            </ul>
