import os

base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
filenames = ['remove_mock.php', 'clear_coords_10957.php', 'swap_coords_10957.php', 'migrate_settings.php']

for root, dirs, files in os.walk(base_dir):
    if 'node_modules' in root or '.git' in root or 'scratch' in root:
        continue
    for file in files:
        if file.endswith('.php') or file.endswith('.js') or file.endswith('.html'):
            path = os.path.join(root, file)
            try:
                with open(path, 'r', encoding='utf-8') as f:
                    content = f.read()
            except Exception:
                try:
                    with open(path, 'r', encoding='tis-620') as f:
                        content = f.read()
                except Exception:
                    continue
            
            for fn in filenames:
                if fn in content:
                    print(f"Found reference to '{fn}' in {os.path.relpath(path, base_dir)}")
